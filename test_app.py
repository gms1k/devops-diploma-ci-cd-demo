...
# Тест для головного маршруту ("/")
def test_read_root():
    # Виконуємо GET-запит до кореневого URL
    response = client.get("/")
    # Перевіряємо, що статус-код відповіді 200 (OK)
    assert response.status_code == 200
    # Перевіряємо, що JSON-відповідь відповідає очікуваному повідомленню
    assert response.json() == {"message": "Hello from FastAPI! This is your DevOps Pipeline."}
